...
%\label{sec:glauber}
Nuclear geometry in heavy-ion collisions is often modelled with the Glauber Model. The model was originally introduced already in 1958~\cite{Glauber:1959}. The approach used in modern applications was introduced in 1976~\cite{Biallas1976461} by Białłas, Bleszyński, and Czyż to model inelastic nuclear collisions
...
\centering
\begin{tikzpicture}
      \draw[->] (0,0) -- (6,0) node[below] {$r\left[\unit{fm}\right]$};
      \draw[->] (0,0) -- (0,5) node[left] {$\nicefrac{\rho}{\rho_0}$};
      \draw	(0,0) node[anchor=north] {0}
		(2,0) node[anchor=north] {5}
		(4,0) node[anchor=north] {10};

      \draw	(0,2) node[anchor=east] {0.5}
		(0,4) node[anchor=east] {1};
	
      \draw[dashed,black] (4,0) -- (4,5);

	
     \draw[black,thin,<->] (0,2) -- node[above] {$R$} (4,2);
     \draw[black,thin,<->] (3.75,1) -- node[below] {$a$} (4.25,1);
     
     \draw[blue] (2,3.5) node {$\omega=0$};
     \draw[blue] (2,4.5) node {$\omega>0$};

      \draw[scale=1,domain=0:6,smooth,variable=\x,blue,dashed] plot ({\x},{(4+1*\x^2/4^2)/(1+exp((\x-4)/0.5))});
       \draw[scale=1,domain=0:6,smooth,variable=\x,blue] plot ({\x},{(4)/(1+exp((\x-4)/0.5))});

      %\draw[scale=0.5,domain=-3:3,smooth,variable=\y,red]  plot ({\y*\y},{\y});

\end{tikzpicture}
\caption{A typical woods-saxon potential}
