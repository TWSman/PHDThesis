...
\caption[Dihadron $\jt{}$ results]{Measured \jt{} signal distribution using dihadron correlations. The distribution is fitted }
...
%At $\jt{} \approx 0.4 \gev$ there is a small bump in the distribution to fit ratio. This was attributed to cases where the trigger particle decayed after hadronisation. As it is difficult to correct for, this bump is included in the systematic errors of the final results.  
...
\noindent  Thus for tracks with $\pt{,track} < \pt{0} $, must be $\jt{} < \pt{0} \cdot R$.  This illustrated in Fig.~\ref{fig:jtmax}
\begin{figure}[h!]
\tikzset{
